    ...
    <section class="section__accordion">
        <div class="component__accordion">
            <div @click="handleToggleAccordionVisible" class="accordion__header">
                <div>
                    <span class="accordion__title">{{ props.index + 1 }}° SORTEIO</span>
                </div>
                <div class="accordion__icon">
                    <div class="accordion__line"></div>
                    <div class="accordion__line accordion__line__vertical"
            ...
            <div>
                <slide-up-down :active="visible" class="wobbly__accordion" :duration="300">
                    <div class="accordion__content">
                        <div v-for="(winner, index) in winner.winners" :key="winner.name + index" class="winner">
                            <p><strong>Nome:</strong> {{ winner.name }}</p>
                            <p><strong>Prêmio:</strong> {{ winner.prize }}</p>
                            <p><strong>Data:</strong> {{ winner.date }}</p>
                        </div>
                    </div>
                </slide-up-down>
            </div>
